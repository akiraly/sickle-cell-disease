...
public class DistributionBasedRandom<T> {
  private final RangeMap<Double, T> distribution;
  private final RandomGenerator randomGenerator;

  public DistributionBasedRandom(RandomGenerator randomGenerator, RangeMap<Double, T> distribution) {
    this.randomGenerator = requireNonNull(randomGenerator);
    this.distribution = requireNonNull(distribution);
  }

  public static RangeMap<Double, Integer> newDistribution(double... ratios) {
    ...
    for (int fi = 0; fi < requireNonNull(ratios).length; fi++) {
      ratiosToValues.add(Maps.immutableEntry(ratios[fi], fi));
    }
    return newDistribution(ratiosToValues.build());
  }

  public static <T> RangeMap<Double, T> newDistribution(Map<Double, T> ratiosToValues) {
    return newDistribution(ratiosToValues.entrySet());
  }

  public static <T> RangeMap<Double, T> newDistribution(Iterable<Map.Entry<Double, T>> ratiosToValues) {
    ...
    for (Map.Entry<Double, T> r : ratiosToValues) {
      ...
      result.put(closedOpen(start, total), r.getValue());
    }
    return result.build();
  }
